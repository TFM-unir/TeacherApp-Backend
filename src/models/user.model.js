...
const updateUserById = (id, { name, nickname, email, phone, password, date_of_birth, status, role_id, location_id, photo }) => {
    return db.query('UPDATE users SET name= ?, nickname= ?, email= ?, phone= ?, password= ?, date_of_birth= ?, status= ?, role_id = ?, location_id= ?, photo= ? WHERE id = ?', [name, nickname, email, phone, password, date_of_birth, status, role_id, location_id, photo, id]);
};
...
module.exports = { selectAllUsers, insertUser, selectUserById, updateUserById, deleteUserById, selectUserByIdWhithOutLocation }